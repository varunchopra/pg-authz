...
-- 2. Find all grants on the resource (direct to user + via groups)
...
AS $$
    ...
    -- Phase 2: Find permissions granted on the resource
    granted_permissions AS (
        -- Direct grants to user
        SELECT relation AS perm
        FROM authz.tuples
        ...
        -- Grants via groups (including nested)
    ...
    -- Phase 3: Expand permission hierarchy
    all_permissions AS (
